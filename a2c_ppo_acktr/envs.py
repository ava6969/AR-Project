...
def make_vec_envs(env_name,
    ...
    if vector_split:
        if gamma is None:
            envs = VecNormalize(envs, norm_reward=False, vector_dim=vector_split)
        else:
            envs = VecNormalize(envs, gamma=gamma, vector_dim=vector_split)
...
class VecNormalize(VecNormalize_):
    def __init__(self, *args, **kwargs):
        self.vector_dim = kwargs.get('vector_dim', None)
        if self.vector_dim:
            kwargs.pop('vector_dim')
        super(VecNormalize, self).__init__(*args, **kwargs)
        self.training = True

    def _obfilt(self, _obs, update=True):
        obs = _obs[:, :self.vector_dim]
        if self.obs_rms:
